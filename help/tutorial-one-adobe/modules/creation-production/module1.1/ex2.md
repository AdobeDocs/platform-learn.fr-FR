...
Dans le cadre de la création de l’automatisation de supply chain de contenu, plusieurs opérations de fichier doivent souvent se produire pour un cas d’utilisation spécifique. Par exemple, il se peut que l’arrière-plan d’un fichier doive être modifié, que le texte de différents calques doive être modifié, etc. Il n’est pas toujours possible d’effectuer toutes les opérations de fichier en même temps, ce qui rend nécessaire une approche à plusieurs étapes. Après chaque étape intermédiaire, la sortie est alors un fichier temporaire qui est nécessaire pour que l’étape suivante soit exécutée. Une fois cette étape suivante exécutée, le fichier temporaire perd rapidement de la valeur et n’est souvent plus nécessaire, il doit donc être supprimé.
...
## 1.1.2.2 Création d’un abonnement Azure
...
Dans le menu de gauche, sélectionnez **Toutes les ressources**. Si vous n’êtes pas encore abonné, l’écran d’abonnement à Azure s’affiche.
...
Remplissez le formulaire d’abonnement Azure et fournissez votre téléphone mobile et votre carte de crédit pour l’activation (vous disposerez d’un niveau gratuit pendant 30 jours et aucun frais ne vous sera facturé, sauf si vous effectuez une mise à niveau).
...
[Téléchargez l’explorateur de stockage Microsoft Azure pour gérer vos fichiers](https://azure.microsoft.com/en-us/products/storage/storage-explorer#Download-4){target="_blank"}. Sélectionnez la version appropriée pour votre système d’exploitation spécifique, téléchargez-la et installez-la.
