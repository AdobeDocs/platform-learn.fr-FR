---
title: Query Service - Requêtes, requêtes, requêtes... et analyse de résiliation
description: Query Service - Requêtes, requêtes, requêtes... et analyse de résiliation
kt: 5342
doc-type: tutorial
...
Dans cette requête, vous joindrez des jeux de données. Dans ce cas, vous joignez le jeu de données `demo_system_event_dataset_for_website_global_v1_1` au jeu de données `demo_system_event_dataset_for_call_center_global_v1_1`. Cela permet de comprendre le résultat de l’interaction du centre d’appels.
...
Ne supprimez pas les `/\*` et les `\*/`. L’instruction modifiée dans le bloc-notes doit se présenter comme suit :
...
Accédez à [2.1.5 Générer un jeu de données à partir d’une requête](./ex5.md){target="_blank"}
