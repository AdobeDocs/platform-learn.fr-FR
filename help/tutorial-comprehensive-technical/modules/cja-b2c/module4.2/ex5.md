---
title: Ingestion et analyse des données des Google Analytics dans Adobe Experience Platform avec le connecteur Source BigQuery - Analyse des données des Google Analytics à l’aide de Customer Journey Analytics
description: Ingestion et analyse des données des Google Analytics dans Adobe Experience Platform avec le connecteur Source BigQuery - Analyse des données des Google Analytics à l’aide de Customer Journey Analytics
kt: 5342
doc-type: tutorial
...
- Connectez-vous et rejoignez des Google Analytics avec les données de fidélité.
- Se familiariser avec l’interface utilisateur de CJA

## 4.2.5.1 Créer une connexion
...
## 4.2.5.2 Créer une vue de données
...
Les tableaux à structure libre sont presque illimités. Vous pouvez faire (presque) n’importe quoi, ce qui est très utile par rapport aux Google Analytics (car cet outil présente certaines limites en matière d’analyse). C’est l’une des raisons pour lesquelles il faut charger les données des Google Analytics dans un autre outil d’analyse.

Découvrez deux exemples dans lesquels vous devez utiliser SQL, BigQuery et du temps pour répondre à des questions simples qui ne sont pas possibles dans l’interface utilisateur Google Analytics ou Google Data Studio :
...
Comme mentionné ci-dessus, les **tableaux à structure libre** vous donnent la liberté dont vous avez besoin pour effectuer une analyse approfondie. Par exemple, vous pouvez sélectionner n’importe quelle autre Dimension pour ventiler une mesure spécifique dans le tableau.
...
Sélectionnez la Dimension **Safari** et glissez-déposez-la en haut d’une mesure, par exemple **Passages en caisse**. Vous verrez alors ceci :
...
Un aspect encore plus spécial est la possibilité de joindre n’importe quel jeu de données de Adobe Experience Platform pour enrichir les données comportementales numériques des Google Analytics. Par exemple, les données hors ligne, de centre d’appel, de fidélité ou de gestion de la relation client.
...
Analysons ensuite quel **type d’appareil** est utilisé par les clients qui se sont rendus sur le site à l’aide de **recherche organique** avec un **niveau de fidélité** qui est **bronze**. Prenez la Dimension **Type d’appareil** et glissez-déposez-la sur **Bronze**. Vous verrez alors ceci :
...
Nous pouvons continuer à fractionner les lignes avec des segments ou des périodes spécifiques (peut-être pour refléter des campagnes TV spécifiques) pour poser des questions aux Customer Journey Analytics et obtenir les réponses en déplacement.

Obtenir le même résultat final avec SQL, puis un outil de visualisation tiers, est un véritable défi. Surtout quand on pose des questions et qu&#39;on essaie d&#39;obtenir des réponses à la volée. Customer Journey Analytics n’est pas confronté à ce défi et permet aux analystes de données d’interroger les données de manière flexible et en temps réel.

## 4.2.5.3.2 Analyse de l’entonnoir ou des abandons
...
Ce type d’analyse n’est pas possible dans l’interface utilisateur de Google Analytics ou est très limité. CJA ajoute donc une fois de plus beaucoup de valeur aux données capturées par les Google Analytics.
...
Vous devez maintenant ajouter des actions d’e-commerce. Dans les Dimensions, recherchez la dimension Dimension **Type d’événement**. Cliquez pour ouvrir la dimension.
...
Si vous souhaitez effectuer une analyse de flux avancée à l’aide de Google Analytics, vous devez utiliser le langage SQL pour extraire les données, puis utiliser une solution tierce pour la partie visualisation. Customer Journey Analytics vous aidera à y parvenir.
...
Recherchez la Dimension **Type d’événement** et faites-la glisser et déposez-la dans la zone **Dimension de sortie**.
...
Analysons plus en détail ! Recherchez la Dimension **Nom de la page**, puis faites-la glisser et déposez-la sur le nouveau chemin d’accès généré.
