...
Dans cette requête, vous allez joindre des jeux de données. Dans ce cas, vous rejoindrez le jeu de données `demo_system_event_dataset_for_website_global_v1_1` avec le jeu de données `demo_system_event_dataset_for_call_center_global_v1_1`. Cela permet de comprendre le résultat de l’interaction du centre d’appels.
...
Dans cette requête, nous joignons les données CRM intégrées à Adobe Experience Platform. Cela permet d’enrichir l’analyse de la perte de clientèle avec des données CRM.
