---
title: Validation des mises en oeuvre de Target avec le SDK Web | Migration de Target depuis at.js 2.x vers le SDK Web
...
* Vérifier que la création de rapports fonctionne correctement
...
| Les paramètres attendus se trouvent dans les objets XDM et de données de la variable `sendEvent` requête |  |
...
| Les paramètres de profil sont transmis à Target avec succès. | Afficher les traces Edge dans Debugger |
...
| Le contenu cible est renvoyé dans le `sendEvent` réponses | Attendu lorsque `renderDecisions` est définie sur `true` Les portées ou sont demandées et l’utilisateur est admissible pour une activité Target particulière. |
| A `decisioning.propositionDisplay` se déclenche après le rendu des activités basées sur VEC. | Les activités rendues automatiquement et à la demande doivent comporter des appels d’événement distincts. |
| A `decisioning.propositionDisplay` se déclenche après le rendu des activités basées sur des formulaires. | Applicable uniquement pour certaines mises en oeuvre. Nécessite du code personnalisé pour exécuter cet appel. |
| A `decisioning.propositionDisplay` se déclenche lorsqu’une offre est appliquée à un changement de vue SPA | Applicable uniquement pour les mises en oeuvre SPA |
| A `decisioning.propositionDisplay` ne se déclenche PAS lorsqu’un composant SPA est rendu pour une vue donnée. | Applicable uniquement pour les mises en oeuvre SPA |
| A `decisioning.propsitionInteract` se déclenche après une conversion d’activité. | Le &quot;clic sur un élément&quot; et les conversions personnalisées ont été migrés depuis at.js. `trackEvent` ou `sendNotifications` doivent comporter des appels d’événement distincts. |
| Les jetons de réponse sont renvoyés dans la variable `sendEvent` réponse et avoir des valeurs attendues | Les jetons de réponse doivent fonctionner normalement avec le SDK Web |
...
| Élément de validation | Notes |
|---|---|
| Les activités basées sur le compositeur d’expérience visuelle s’affichent correctement au chargement de la page. | Il est préférable de valider les modifications de code personnalisé et les modifications de base, telles que la réorganisation des éléments ou le remplacement de texte. |
...
| Les liens d’assurance qualité fonctionnent comme prévu | Si cela ne fonctionne pas, vérifiez que le fichier web.webPageDetails.URL correspond exactement à l’URL du navigateur. |
...
| Élément de validation | Notes |
|---|---|
| Les visiteurs sont attribués aux activités basées sur le compositeur d’expérience visuelle | Il est préférable de valider le fonctionnement des rapports comme prévu pour les modifications de chargement de page et les modifications d’affichage SPA |
| Les visiteurs sont attribués aux activités d’après les formulaires | Selon l’approche de rendu utilisée, votre mise en oeuvre peut nécessiter un code personnalisé pour exécuter une `decisioning.propositionDisplay` event |
...
| Élément de validation | Notes |
|---|---|
| Les audiences utilisées dans les activités en direct sont compatibles avec le SDK Web de Platform. | Les audiences qui utilisent le composant &quot;Personnalisé&quot; (paramètre de mbox) doivent être mises à jour afin d’inclure les attributs XDM. |
...
| Les scripts de profil évaluent comme prévu | Afficher les traces Edge dans Debugger |

## Intégrations aux applications Adobe

| Élément de validation | Notes |
|---|---|
| Retour des activités pour les audiences Experience Cloud | Par exemple, un segment publié à partir d’Adobe Analytics |
...
| Les données d’activité de Target s’affichent dans Analysis Workspace | S’applique aux activités qui utilisent Adobe Analytics comme source de création de rapports. |
...
| Les informations provenant de tiers sont transmises en tant que données XDM ou de profil. | Toutes les informations pertinentes provenant de tiers doivent être transmises `sendEvent` appels à Target |
...
Ensuite, apprenez à [résolution des problèmes liés à une mise en oeuvre de Target à l’aide du SDK Web Platform](debugging.md).
