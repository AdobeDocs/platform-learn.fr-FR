...
* Plus difficile d’utiliser la couche de données pour les pixels tiers (mais vous souhaiterez peut-être déplacer ces pixels vers le [transfert d’événement](setup-event-forwarding.md) !
...
Avant de créer l’objet XDM, créez l’ensemble suivant d’éléments de données pour la couche de données [Luma demo site](https://luma.enablementadobe.com/content/luma/us/en.html){target="_blank"} :

1. Accédez à **[!UICONTROL Data Elements]** et sélectionnez **[!UICONTROL Add Data Element]** (ou **[!UICONTROL Create New Data Element]** s’il n’existe aucun élément de données dans la propriété de balise).

   ![Créer un élément de données](assets/data-element-create.png)

1. Nommez l’élément de données `page.pageInfo.pageName`.
1. Utilisez la **[!UICONTROL variable JavaScript]** **[!UICONTROL type d’élément de données]** pour pointer vers une valeur dans la couche de données de Luma : `digitalData.page.pageInfo.pageName`
...
>[!CAUTION]
>
>Le type d’élément de données [!UICONTROL JavaScript variable] traite les références de tableau comme des points au lieu de crochets. Par conséquent, le référencement de l’élément de données du nom d’utilisateur comme `digitalData.user[0].profile[0].attributes.username` **ne fonctionnera pas**.

## Création d’éléments de données de variable pour XDM et les objets de données

Les éléments de données que vous venez de créer seront utilisés pour créer un objet XDM (pour les applications Platform) et un objet de données (pour Analytics, Target et Audience Manager). Ces objets ont leurs propres éléments de données spéciaux appelés **[!UICONTROL Variable]** qui sont très faciles à créer.

Pour créer l’élément de données Variable pour XDM, vous l’associez au schéma que vous avez créé dans la leçon [Configurer un schéma](configure-schemas.md) :

1. Sélectionnez **[!UICONTROL Ajouter un élément de données]**
1. Nommez votre élément de données `xdm.variable.content`. Il est recommandé d’ajouter le préfixe &quot;xdm&quot; aux éléments de données spécifiques à XDM pour mieux organiser la propriété de balise.
1. Sélectionnez le **[!UICONTROL SDK Web Adobe Experience Platform]** comme **[!UICONTROL extension]**
...
1. Sélectionnez le **[!UICONTROL Schéma]** approprié, ici `Luma Web Event Data`
1. Sélectionnez **[!UICONTROL Save]**
...
1. Sélectionnez **[!UICONTROL Ajouter un élément de données]**
1. Nommez votre élément de données `data.variable`. Il est recommandé d’ajouter le préfixe &quot;data&quot; aux éléments de données spécifiques à l’objet de données pour mieux organiser la propriété de balise.
1. Sélectionnez le **[!UICONTROL SDK Web Adobe Experience Platform]** comme **[!UICONTROL extension]**
1. Sélectionnez **[!UICONTROL Variable]** comme **[!UICONTROL Type d’élément de données]**
...
1. Sélectionnez **[!UICONTROL Save]**
   ...
   ![ Elément de données variable pour l’objet de données ](assets/data-element-data-variable.png.png)
...
>[!TIP]
>
>Dans une leçon ultérieure [Créer des règles de balise](create-tag-rule.md), vous découvrirez comment les éléments de données **[!UICONTROL Variable]** vous permettront d’empiler plusieurs règles dans des balises à l’aide du **[!UICONTROL type d’action de mise à jour de variable]**.
