...
[Audiences](https://experienceleague.adobe.com/docs/core-services/interface/audiences/audience-library.html?lang=fr) fait partie du service principal People et vous permet de partager des audiences entre différentes solutions. Par exemple, vous pouvez créer une audience dans Audience Manager et l’utiliser pour diffuser du contenu personnalisé avec Target.
...
1. La mise en œuvre d’Adobe Experience Platform Identity Service
1. Définition des ID de client via le service d’ID *before* Target et Analytics déclenchent leurs requêtes (ce que vous avez accompli à l’aide de la fonction d’agencement des règles dans les balises).
